...
commitDateAndTime=`git log -n 1 --pretty=format:"%cd" --date=iso $MOD_DIR`
commitDateYear=`echo $commitDateAndTime | cut -d- -f1 | cut -d' ' -f1`
commitDateMonth=`echo $commitDateAndTime | cut -d- -f2 | cut -d' ' -f1`
commitDateDay=`echo $commitDateAndTime | cut -d- -f3 | cut -d' ' -f1`
commitTimeHour=`echo $commitDateAndTime | cut -d: -f1 | cut -d' ' -f2`
commitTimeMin=`echo $commitDateAndTime | cut -d: -f2 | cut -d' ' -f2`
...
\"$MAJOR_REVISION.$MINOR_REVISION.${commitDateYear}${commitDateMonth}${commitDateDay}${commitTimeHour}${commitTimeMin}\"" >> $MOD_SRC_INC_DIR/revision.h
echo "" >> $MOD_SRC_INC_DIR/revision.h
